...
public class TheaHttpClientFactory : IHttpClientFactory, IHttpMessageHandlerFactory
{
    private static readonly TimerCallback _cleanupCallback = (s) => ((TheaHttpClientFactory)s!).CleanupTimer_Tick();
    private readonly ILogger _logger;
    private readonly IServiceProvider _services;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly IOptionsMonitor<HttpClientFactoryOptions> _optionsMonitor;
    private readonly IHttpMessageHandlerBuilderFilter[] _filters;
    private readonly Func<string, Lazy<ActiveHandlerTrackingEntry>> _defaultEntryFactory;
    private readonly Func<string, string, string, string, bool, Lazy<ActiveHandlerTrackingEntry>> _proxyEntryFactory;
    ...
    private readonly ConcurrentDictionary<int, Lazy<ActiveHandlerTrackingEntry>> _proxyHandlers;
        ...
        IEnumerable<IHttpMessageHandlerBuilderFilter> filters)
    {
        ...
        _logger = loggerFactory.CreateLogger<TheaHttpClientFactory>();

        // case-sensitive because named options is.
        _activeHandlers = new ConcurrentDictionary<string, Lazy<ActiveHandlerTrackingEntry>>(StringComparer.Ordinal);
        _proxyHandlers = new ConcurrentDictionary<int, Lazy<ActiveHandlerTrackingEntry>>();

        _defaultEntryFactory = (name) =>
        {
            return new Lazy<ActiveHandlerTrackingEntry>(() =>
            {
                return CreateDefaultHandlerEntry(name);
            }, LazyThreadSafetyMode.ExecutionAndPublication);
        };
        _proxyEntryFactory = (name, proxyAddress, proxyUser, proxyPassword, isIgnoreSslError) =>
        {
            return new Lazy<ActiveHandlerTrackingEntry>(() =>
            {
                return CreateProxyHandlerEntry(name, proxyAddress, proxyUser, proxyPassword, isIgnoreSslError);
            }, LazyThreadSafetyMode.ExecutionAndPublication);
        };
    ...
    public HttpClient CreateClient(string name, string proxyAddress, string proxyUser = null, string proxyPassword = null, bool isIgnoreSslError = true)
    {
        if (string.IsNullOrEmpty(name))
        ...
        var handler = CreateProxyHandler(name, proxyAddress, proxyUser, proxyPassword, isIgnoreSslError);
        var client = new HttpClient(handler, disposeHandler: false);
    ...
    public HttpMessageHandler CreateHandler(string name)
    {
        ...
        ActiveHandlerTrackingEntry entry = _activeHandlers.GetOrAdd(name, _defaultEntryFactory).Value;

        StartHandlerEntryTimer(entry);

        return entry.Handler;
    }
    public HttpMessageHandler CreateProxyHandler(string name, string proxyAddress, string proxyUser = null, string proxyPassword = null, bool isIgnoreSslError = true)
    {
        if (string.IsNullOrEmpty(name))
            throw new ArgumentNullException(nameof(name));

        var hashKey = HashCode.Combine(name, proxyAddress);
        var proxyEntry = _proxyEntryFactory(name, proxyAddress, proxyUser, proxyPassword, isIgnoreSslError);
        ActiveHandlerTrackingEntry entry = _proxyHandlers.GetOrAdd(hashKey, proxyEntry).Value;
    ...
    // Internal for tests
    ActiveHandlerTrackingEntry CreateDefaultHandlerEntry(string name)
    {
    ...
    ActiveHandlerTrackingEntry CreateProxyHandlerEntry(string name, string proxyAddress, string proxyUser = null, string proxyPassword = null, bool isIgnoreSslError = true)
    {
        IServiceProvider services = _services;
        IServiceScope scope = null;
        ...
        try
        {
            var builder = services.GetRequiredService<ProxyHttpMessageHandlerBuilder>();
            builder.Name = name;
            builder.ProxyAddress = proxyAddress;
            builder.User = proxyUser;
            builder.Password = proxyPassword;
            builder.IsIgnoreSslError = isIgnoreSslError;
    ...
    void StartHandlerEntryTimer(ActiveHandlerTrackingEntry entry)
    {
    ...
    sealed class ActiveHandlerTrackingEntry
    {
    ...
    sealed class LifetimeTrackingHttpMessageHandler : DelegatingHandler
    {
    ...
    sealed class ExpiredHandlerTrackingEntry
    {
